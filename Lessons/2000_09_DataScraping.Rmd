...
The U.S. Environmental Protection Agency maintains a [website](https://www.epa.gov/nutrient-policy-data/estimated-total-nitrogen-and-total-phosphorus-loads-and-yields-generated-within) that summarizes water quality assessment reports from each state, detailing waters impaired due to nutrient-related causes under Sections 305(b) and 303(d) of the Clean Water Act. There are several useful tables on this website: 
...
url <- "https://www.epa.gov/nutrient-policy-data/estimated-total-nitrogen-and-total-phosphorus-loads-and-yields-generated-within"
...
State <- webpage %>% html_nodes("td:nth-child(1)") %>% html_text()
Net.TN.load <- webpage %>% html_nodes("td:nth-child(2)") %>% html_text()
Avg.TN.load <- webpage %>% html_nodes("td:nth-child(3)") %>% html_text()
Net.TP.load <- webpage %>% html_nodes("td:nth-child(4)") %>% html_text()
Avg.TP.load <- webpage %>% html_nodes("td:nth-child(5)") %>% html_text()


# Note: each of these objects has 50 items in it; consistency is good!
...
NP_loads <- data.frame(State, Net.TN.load, Avg.TN.load, Net.TP.load, Avg.TP.load)
```

Notice there are some instances where symbols appear in cells where there should just be numbers or NAs. Let's fix this. 

```{r fix.values}
fix.values <- function(x){
  x <- as.numeric(gsub("\\,","", x))
}
NP_loads$Net.TN.load <- fix.values(NP_loads$Net.TN.load)


NP_loads$Net.TN.load <- str_replace(NP_loads$Net.TN.load, pattern = "([,])", replacement = "")  
NP_loads$Avg.TN.load <- str_replace(NP_loads$Avg.TN.load, pattern = "([,])", replacement = "")  
NP_loads$Net.TP.load <- str_replace(NP_loads$Net.TP.load, pattern = "([,])", replacement = "")  
NP_loads$Avg.TP.load <- str_replace(NP_loads$Avg.TP.load, pattern = "([,])", replacement = "")  
```
