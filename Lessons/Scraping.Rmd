...
scrape.it <- function(the_year, the_facility){
  ...
  avg_withdrawals <- the_website %>% html_nodes(the_data_id) %>% html_text()
...
## Scrape 2020 water usage data for all sites 
...
the_dfs <- cross2(the_years,the_facilities) %>% 
  map(lift(scrape.it))

#Combine the dataframes into single one
the_df <- do.call("rbind",the_dfs)

#Plot
ggplot(the_df,aes(x=Date,y=Withdrawals_mgd,color=Registrant)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
